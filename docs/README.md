...
- [x] 레이싱 게임 진행
  - [x] 매 턴마다 0~9 사이의 random 값을 구함
  - [x] 4 이상이 나온 경우 전진하고 3 이하인 경우 정지
  - [x] 진행된 라운드마다 이동거리 출력을 반복
    - [x] 각 자동차 이름과 이동거리를 "-"의 길이로 출력
  
- [x] 우승한 자동차 이름 출력
  - [x] 단독 우승이면 단독 출력
  - [x] 공동 우승이면 ","로 구분하여 출력
  - 예외처리
    - [x] 모든 자동차의 이동거리가 0인 경우 우승자가 없음
